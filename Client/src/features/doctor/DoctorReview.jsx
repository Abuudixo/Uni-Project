...
export default function DoctorReview({ patient, scores, onBack, onDelete }) {
  ...
  const handleDownloadReport = () => {
    const content = `MIND BRIDGE - CLINICAL REPORT\n` +
      ...
      `ASSESSMENT SCORES\n` +
      `-----------------\n` +
      (phqScore !== null ? `PHQ-9 (Mental Health): ${phqScore}/${ASSESSMENTS.phq9.maxScore} (${phqSev.label})\n` : "PHQ-9: Not submitted\n") +
      (gadScore !== null ? `GAD-7 (Generalized Anxiety): ${gadScore}/${ASSESSMENTS.gad7.maxScore} (${gadSev.label})\n` : "GAD-7: Not submitted\n") +
      `\nReport generated by MindBridge Medical Portal.\n`;
    
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `Report_${patient.lastName}_${patient.firstName}.txt`;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.cream, fontFamily: font }}>
      <div style={{ background: "linear-gradient(150deg,#1C3050 0%,#3562A0 100%)", padding: "44px 20px 20px", color: "#fff" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, opacity: 0.8 }}>{patient.pronouns || "New patient"}</div>
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 18px 24px" }}>
        ...
        {/* Footer Actions */}
        <div style={{ marginTop: 40, borderTop: `1px solid ${C.border}`, paddingTop: 24, display: "flex", flexDirection: "column", gap: 12 }}>
          <Btn onClick={handleDownloadReport} variant="secondary" style={{ width: "100%" }}>📄 Export Clinical Report (PDF)</Btn>
          <button onClick={onDelete} style={{ background: C.dangerLight, color: C.danger, border: `1px solid ${C.danger}44`, borderRadius: 12, padding: "14px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>🗑 Delete Patient Record</button>
          <Btn onClick={onBack} variant="ghost" style={{ width: "100%" }}>← Return to Patient List</Btn>
        </div>
