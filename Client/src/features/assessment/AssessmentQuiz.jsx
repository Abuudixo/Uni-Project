...
export default function AssessmentQuiz({ assessmentKey, onDone, onBack }) {
  ...
  const handleDownload = () => {
    const content = `MIND BRIDGE - ASSESSMENT RESULTS\n` +
      `Date: ${new Date().toLocaleDateString()}\n\n` +
      `ASSESSMENT: ${a.name}\n` +
      `Score: ${score}/${a.maxScore}\n` +
      `Severity: ${sev.label}\n\n` +
      `This report has been shared with your provider.\n`;
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `Results_${a.name}.txt`;
  ...
  if (finished) {
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.cream, fontFamily: font }}>
        ...
        <div style={{ padding: "12px 18px 22px", borderTop: `1px solid ${C.border}`, background: C.white, display: "flex", gap: 10 }}>
          <Btn onClick={handleDownload} variant="secondary" style={{ flex: 1 }}>📥 Download</Btn>
          <Btn onClick={() => onDone(score)} style={{ flex: 1 }}>Finish & Close</Btn>
        </div>
      </div>
    );
  }
